namespace AcadLib.Tables.Google
{
    ...
    public class GoogleTable
    {
        ...
        {
            return _table.Rows.Select(s =>
            {
                var values = Enumerable.Range(1, _table.Columns.Count).Select(_ => new CellData()).ToArray();
                foreach (var c in s)
                    values[c.Column] = GetCell(_table.Cells[c.Row, c.Column]);
            ...
            foreach (var r in _table.Cells)
            {
                ...
                yield return new Request
                {
                    MergeCells = new MergeCellsRequest
                    {
                        ...
                        {
                            SheetId = _sheetId,
                            StartRowIndex = m.TopRow,
                            EndRowIndex = m.BottomRow + 1,
